...
export function Layout({ children }: { children: ReactNode }) {
  ...
  // Ensure in-page hash links scroll correctly even with React Router
  useEffect(() => {
    // Delay to ensure content is in the DOM
    const t = setTimeout(() => {
      if (hash) {
        scrollToHash(hash);
      } else {
        // If navigating to path without hash, optionally restore to top
        // This mimics native browser behavior for links like "/"
        // Comment out if you want to preserve scroll
  ...
  // Intercept same-page hash link clicks so they scroll smoothly on first click
  useEffect(() => {
    const onClick = (e: MouseEvent) => {
      ...
      // Determine if this is a same-page hash link
      ...
      // Internal same-page hash navigation: prevent default and smooth scroll
      e.preventDefault();
      const id = targetHash.replace('#', '');
      // Update the URL hash via router for proper history behavior
      navigate(`${location.pathname}#${id}`);
      // Ensure scroll happens immediately on first click
      setTimeout(() => scrollToHash(`#${id}`), 0);
    };
